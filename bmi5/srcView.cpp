...
{
	g_return_if_fail (GTK_IS_TOGGLE_ACTION (action) && GTK_SOURCE_IS_VIEW (user_data));
	gtk_text_view_set_wrap_mode ((GtkTextView*)user_data,
				     gtk_toggle_action_get_active (GTK_TOGGLE_ACTION (action)) ?
			   ...
			   gpointer user_data)
{
	...
	gtk_source_view_set_smart_home_end (
		GTK_SOURCE_VIEW (user_data),
		(GtkSourceSmartHomeEndType)gtk_radio_action_get_current_value 
	       ...
	       GtkTextSearchFlags   *flags_p)
{
	...
	entry1 = (GtkEntry*)g_object_new (GTK_TYPE_ENTRY,
			       "visible", TRUE,
	...
	entry2 = (GtkEntry*)g_object_new (GTK_TYPE_ENTRY,
			       "visible", replace,
	...
	case_sensitive = (GtkToggleButton*)g_object_new (GTK_TYPE_CHECK_BUTTON,
				       "visible", TRUE,
	...
	*replacement_p = search_data.replacement = g_strdup (gtk_entry_get_text (entry2));
	search_data.flags = (GtkTextSearchFlags)(gtk_toggle_button_get_active (case_sensitive) ?
					0 : (int)GTK_TEXT_SEARCH_CASE_INSENSITIVE);
	*flags_p = (GtkTextSearchFlags)search_data.flags; 
	gtk_widget_destroy (dialog);
	return TRUE;
}
...
find_cb (GtkAction *,
	 gpointer   user_data)
{
	do_search_replace ((GtkTextView*)user_data, FALSE);
}

static void
replace_cb (GtkAction *,
	    gpointer   user_data)
{
	do_search_replace ((GtkTextView*)user_data, TRUE);
}

static void
open_file_cb (GtkAction *, gpointer user_data)
{
	GtkWidget *chooser;
	gint response;
...
static void
print_file_cb (GtkAction *, gpointer user_data)
{
	GtkSourceView *view;
	GtkSourceBuffer *buffer;
	GtkSourcePrintCompositor *compositor;
	...
	filename = (const char*)g_object_get_data (G_OBJECT (buffer), "filename");
                 ...
                 gpointer                  *data G_GNUC_UNUSED)
{
	...
	{
		g_print ("Matched bracket: '%c' at row: %d, col: %d\n",
...
static gchar *
mark_tooltip_func (GtkSourceMarkAttributes *,
                   GtkSourceMark           *mark,
                   GtkSourceView*)
{
	GtkTextBuffer *buf;
...
create_main_window (GtkWidget* top, GtkSourceBuffer *buffer)
{
	...
	action_group = (GtkActionGroup*)g_list_nth_data (groups, 0);
...
{
	char *mem = (char*)malloc (n_bytes + ALIGN);
	    ...
	    gsize    n_bytes)
{
	if (mem)
	{
		char *new_mem = (char*)realloc ((char*) mem - ALIGN, n_bytes + ALIGN);
...
my_strdup (const char *s)
{
	if (s)
	{
		char *new_s = (char*)my_malloc (strlen (s) + 1);
		strcpy (new_s, s);
		return new_s;
	}
	else
...
int main_sv(int argc, char **argv)
{
	...
	gchar *builtin_lang_dirs[] = {(char*)"/usr/share/gtksourceview-3.0/language-specs", NULL};
	gchar *builtin_sm_dirs[] = {(char*)"/usr/share/gtksourceview-3.0/styles", NULL};
	gchar **dirs;
