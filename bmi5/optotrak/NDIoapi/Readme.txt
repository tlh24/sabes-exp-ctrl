...
To install the Optotrak API for the Linux platform:
    ...
    5. Then you must set and export shell environment variable ND_DIR
       ...
       user logs in:
      		...
      		b. cd to the user's home directory using the command:
            		cd
      		c. If the user's default shell is the bach shell, then the user's home directory 
    ...
    6. Edit the following entry in optotrak.ini to the interface you are physically connected through:
		...
		[Optotrak System]
		LinkDriver=/usr/NDIoapi/ndigital/pcilink.so  (or etherlink.so, scsilink.so )
...
2. If you type 'optsetup' and you get a message that starts with 'Errors Detected', but 
   ...
   /usr/NDIoapi/ndigital/settings/optotrak.ini.  Edit the 
   /usr/NDIoapi/ndigital/settings/optotrak.ini file, enter LinkDriver=/usr/NDIoapi/ndigital/pcilink.so and re-run
   optsetup.
