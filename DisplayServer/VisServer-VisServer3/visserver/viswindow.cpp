...
void CVisWindow::DrawCounter()
{
	...
	static int cw=0;
	static int ch=0;
	static int ppon;  // pixels per bit
	static int ppoff;  // pixels per bit
	int i,j,k,b, pixon;
	
	if(newactions) actionCount++; 
	newactions = 0;
	...
	if(cx!=counterX||cy!=counterY||ch!=counterH||cw!=counterW){
		...
		ppoff = (int)ceil((double)cw / 
				(double)(VISWIN_COUNTER_BITS*VISWIN_COUNTER_ONOFF_FRAC + VISWIN_COUNTER_BITS-1));
		ppon  = (int)ceil(ppoff*VISWIN_COUNTER_ONOFF_FRAC);
		cw = ppon*VISWIN_COUNTER_BITS + ppoff*(VISWIN_COUNTER_BITS-1);
	...
	for(k=i=0;i<ch;i++){
		for(j=0;j<cw;j++){
			if(VISWIN_COUNTER_L2R) b = j/(ppon+ppoff);
			else                   b = VISWIN_COUNTER_BITS-1-(j/(ppon+ppoff));


			pixon = (b<VISWIN_COUNTER_BITS) & // In case there is overrun due to 4-byte bdry
				(j%(ppon+ppoff)<ppon)       & // Check if on or between counter spots
