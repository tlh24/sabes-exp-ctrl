...
CVMotion::CVMotion(){
	...
	exact = 1;
	pointSize   = 1.0;
	m_dScale[0] = m_dScale[1] = 10.0;  // Set Default Radius to 10
...
CVMotion::~CVMotion(){
	...
	if(scrVertices)		FreeDoubleArray(scrVertices);
...
CVMotion::m_sampleVertices()
{
	...
	scrVertices = CreateDoubleArray(dim, dimlen);
	...
	for (p=vertices, i=0; i<nVertices; p+=2, ++i){
		...
		*/
		...
		if( lifetime>0 && vertexTimes[i]>lifetime ){
			...
			p[0] += ivelX * dt/m_dScale[0];
			p[1] += ivelY * dt/m_dScale[0];
			if(circle){
...
void CVMotion::m_xformVertices(void)
{
	...
	for (p=vertices, sp=scrVertices, i=0; i<nVertices; p+=2, sp+=2, ++i){
		...
		g_TransformCoordinatesD( x, y, sp, sp+1 );
		m_nClipped += IS_OFF_WINDOW(sp[0],sp[1]);
	}
}
...
CVMotion::Draw()
{
	...
	glVertexPointer(GetLength(vert, 0), GL_DOUBLE, 0, vert);
	glDrawArrays(GL_POINTS, 0, GetLength(vert, 1));
