...
//==============================================================================
STDMETHODIMP CVMain::ScaleGroup(long iGroupID, VARIANT vScale, long iLogID){
  int iObj, nObjects, dim;
  ...
  for( iObj = 0; iObj<nObjects; iObj++){
    g_ptObjectList.GetPointer( iObj, (void **) &pObj);
    if(iGroupID) pObj->Scale(dScale[0], dScale[1], iGroupID );
    else pObj->Scale(dScale[0],dScale[1]);
  }
