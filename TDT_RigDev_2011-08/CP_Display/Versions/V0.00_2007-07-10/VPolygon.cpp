...
//==============================================================================
STDMETHODIMP CVPolygon::GetVertices(SAFEARRAY ** ppsaVertices){
  SAFEARRAY *psaMtx;
  double * pdData;

  * ppsaVertices = NULL;
  if( g_MakeSafeArray( 2, m_nVertices, &psaMtx, &pdData ) ){
...
//==============================================================================
STDMETHODIMP CVPolygon::SetVertices(SAFEARRAY * psaVertices, long iLogID){
  double * pdData;
  int nVertices;

  // We expect 2xN array of doubles
  if( g_CheckSafeArray( 2, &nVertices, psaVertices, &pdData )) {
    return g_ErrorToHresult( 1 );
  }
  if( nVertices < 3 ) {
    return g_ErrorToHresult( 1 );
  ...
  SafeArrayUnaccessData( psaVertices );
...
// We have to use 1-D SAFEARRAY. Actual dimesions are
// N_pix_Width x N_pix_Height x N_RGBA, values are doubles
STDMETHODIMP CVPolygon::GetTexture( SAFEARRAY ** ppsaTexture,
                                    long * pnPixelsW,
                                    long * pnPixelsH,
  ...
  * pdTexturePhysWidth = m_dTexturePhysWidth;
  ...
  if( g_MakeSafeArray( nVals, &psaMtx, &pdData ) ){
    return g_ErrorToHresult(1);
  }
...
//==============================================================================
STDMETHODIMP CVPolygon::SetTexture( SAFEARRAY * psaTexture,
  ...
  if( g_CheckSafeArray( &nPixelsH, &nPixelsW, N_RGBA, psaTexture, &pdData )) {
    return g_ErrorToHresult( 1 );
  }
  ...
  if( !(iW_Ok && iH_Ok )){
    CP_printf("Invalid texture size: %d x %d\nTexture size should be a power of 2\n", 
                                nPixelsW, nPixelsH);
    SafeArrayUnaccessData( psaTexture );
    return g_ErrorToHresult( 1 );
  }
