function H = createArrowField(NarrowsX,NarrowsY,wsBounds, Z,visServerName)
...
clear H
for i = 1:Narrows
    H(i) = actxserver('CP_Display.VPolygon', visServerName);
    H(i).SetVertices(aVerts');% + repmat(aLocs(:,i)',size(aVerts,1),1);
    H(i).SetFillFlag(1);
    H(i).Hide;
    H(i).SetXY(aLocs(:,i)');
    H(i).SetZ(Z);
