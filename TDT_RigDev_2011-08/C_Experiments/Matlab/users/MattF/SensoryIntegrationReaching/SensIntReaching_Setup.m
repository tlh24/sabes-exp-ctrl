...
%         hFng      = actxserver('CP_Display.VDisk',strDisplayComputer); 
        hFng      = actxserver('CP_Display.VPolygon',strDisplayComputer);         
        hFng.SetVertices([cos(ang); sin(ang)]);
    end
    if(isempty(hFngRing) | ~iscom(hFngRing)),      % Feedback finger handle
        fprintf('...5 ')
        hFngRing = actxserver('CP_Display.VPolygon',strDisplayComputer);         
        hFngRing.SetVertices([cos(ang); sin(ang)]);
    end
