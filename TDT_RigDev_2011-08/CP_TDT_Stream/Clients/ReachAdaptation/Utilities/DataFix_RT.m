...
while (directory)
    try,
        ...
        cd(sprintf('ReachTuning_%s-%d_TEMP',namehold,filect));
        filect=filect+1;
    catch,
        fprintf(1,'Directory not found\n',filect)
        lastFile = filect-1;
        break;
    end
    fnames = dir;
    ...
    %% Load Data
    load VARS
    if(filect-1==1), % Get start training info from very first VARS file
    ...
    % Save
    if exist('DATA'),
        fprintf(1,'Saving file ReachTuning_%s-%d.mat \n\n',name,number)
        eval(sprintf('save ReachTuning_%s-%d.mat DATA Params TimeStamp', name,number));
        number = number+1;
        clear DATA;
    else
...
eval(sprintf('load ReachTuning_%s-%d_TEMP/DATA_%d.mat',namehold,lastFile,lastTrial));
