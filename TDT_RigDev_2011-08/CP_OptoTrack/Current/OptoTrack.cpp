...
int COptoTrack::GetSignalDistortion( int aiDistortion[N_SENSORS_MAX] ){
  ...
  // Distorsions are updated in the first m_nSensorsToWrite sensors
  // others are set to ecSensorOff in the init functions
  for(i =0; i < N_SENSORS_MAX; i++)
...
//  uFrameNumber++; // Our frames start with 1, Opto frames start with 0 (?)
...
//  CP_printf("Frame Number: %8u\r", uFrameNumber );
  ...
  // If the first read - ignore missed frames 
