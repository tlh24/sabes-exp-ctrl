...
/*
  blnRes = m_PdiDev.LastPnoPtr( pBuffer,  dwSize); // no need for '&' - it's C++ 
  // check for errors
  if(!blnRes) {
    CP_printf("\nWarning: failed to read last buffer\n");
    CP_printf("%s     Err code: %d\n", m_PdiDev.GetLastResultStr(), m_PdiDev.GetLastResult()); 
    return 0;
  }
