...
char * g_szVersion = "CP_LibertyTrack: Ver 0.01 September 6th, 2007";
...
void OnTimer( HWND hDlgWnd ){
  ...
  if( g_GetLatestCoordinates( &nSensors, &dT_ms, adXYZ ) ) return; 
...
BOOL CALLBACK x_DialogProc(HWND hDlgWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) { 
  ...
  switch (uMsg) {
    ...
    DefWindowProc( hDlgWnd, uMsg, wParam, lParam); 
    return 0;
} 
//============================================================================================
//   MAIN
...
/*
  ...
  while ( 1 ) {
    ...
    // Process Liberty messages before processing others
    // This way we make sure that we get the latest coordinates
    while( PeekMessage(&msg, NULL, WM_PI_DATA_STOPPED, WM_PI_RAWDATA_WARNING, PM_REMOVE)){ 
      TranslateMessage( &msg ); 
      DispatchMessage( &msg );
    }

    // Process other messages
    // Note: all DCOM calls are made via the uMsg loop
    //    while ( GetMessage( &msg, NULL, 0, 0 )  ) {
    // Process dialog messages in IsDialogMessage() pass others to translate/dispatch
    GetMessage( &msg, NULL, 0, 0 );
    if ( msg.uMsg == WM_QUIT ) goto AppExit;  // Get out on the WM_QUIT uMsg
    //      if(!IsDialogMessage(hDlgWnd,&msg)){
      TranslateMessage(&msg);
      DispatchMessage(&msg); 
    //      }
