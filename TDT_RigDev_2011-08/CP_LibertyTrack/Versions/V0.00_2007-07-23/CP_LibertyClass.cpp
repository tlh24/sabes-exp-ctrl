...
HRESULT CP_LibertyClass::SetSensorsTransformIdx( SAFEARRAY * psaTransformIdx){
  ...
  // We expect array of ints nSensors long
...
HRESULT CP_LibertyClass::GetSensorBuffered( long iSensorIdx,
  ...
  *ppsaVelocity = NULL;
  ...
  if( g_MakeSafeArray( N_COORD * 2, nFrames, &psaCoord, &pfCoord )){
    iErr = 1;
    goto errCleanUp; 
  }
  // Velocities
  // Create output SAFEARRAY of floats, size nFrames x 3 
  if( g_MakeSafeArray( N_COORD, nFrames, &psaVelocity, &pfVelocity )){
    iErr = 1;
    goto errCleanUp; 
  }
  ...
  g_pLibertyTrack->ParseBuffer(iSensor,
                              ...
                              (tfXYZTag *) pfCoord,      // Dest. coordinates and angles
                              (tfXYZTag *) pfVelocity ); // Dest. velocities
...
HRESULT CP_LibertyClass::GetAllSensorsBuffered( long iDataIdx,
  ...
  *ppsaVelocity = NULL;
  ...
  if( g_MakeSafeArray( N_COORD * 2 * nSensors, nFrames, &psaCoord, &pfCoord )){
    iErr = 1;
    goto errCleanUp; 
  }
  // Velocities
  // Create output SAFEARRAY of floats, size nFrames x 3 
  if( g_MakeSafeArray( N_COORD * nSensors, nFrames, &psaVelocity, &pfVelocity )){
    iErr = 1;
    goto errCleanUp; 
  }
  ...
  g_pLibertyTrack->ParseBuffer(nFrames, 
                               pBuf,                     // Source buffer
                               pdTimeStamps,             // Dest. timestamps
                              (tfXYZTag *) pfCoord,      // Dest. coordinates
                              (tfXYZTag *) pfVelocity ); // Dest. velocities
