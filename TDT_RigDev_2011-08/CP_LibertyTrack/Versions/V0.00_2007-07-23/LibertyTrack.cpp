...
CLibertyTrack::CLibertyTrack(){
  ...
  for( iTrans = 0; iTrans<N_TRANSFORMS; iTrans++ ) {
    m_atTransformMtx[iTrans][0][0] = 1;     // X to X'
    m_atTransformMtx[iTrans][0][1] = 0;     // Y to X'
    m_atTransformMtx[iTrans][0][2] = 0;     // Z to X'
    m_atTransformMtx[iTrans][0][3] = 0;     // X0
    m_atTransformMtx[iTrans][1][0] = 0;     // X to Y'
    m_atTransformMtx[iTrans][1][1] = 1;     // Y to Y'
    m_atTransformMtx[iTrans][1][2] = 0;     // Z to Y'
    m_atTransformMtx[iTrans][1][3] = 0;     // Y0
    m_atTransformMtx[iTrans][2][0] = 0;     // X to Z'
    m_atTransformMtx[iTrans][2][1] = 0;     // Y to Z'
    m_atTransformMtx[iTrans][2][2] = 1;     // Z to Z'
    m_atTransformMtx[iTrans][2][3] = 0;     // Z0
  }
  // Swap X and Z axes in the first (default) transform
    m_atTransformMtx[0][0][0] = 0;     // X to X'
    m_atTransformMtx[0][0][1] = 0;     // Y to X'
    m_atTransformMtx[0][0][2] = 1;     // Z to X'
    m_atTransformMtx[0][0][3] = 0;     // X0
    m_atTransformMtx[0][1][0] = 0;     // X to Y'
    m_atTransformMtx[0][1][1] = 1;     // Y to Y'
    m_atTransformMtx[0][1][2] = 0;     // Z to Y'
    m_atTransformMtx[0][1][3] = 0;     // Y0
    m_atTransformMtx[0][2][0] = 1;     // X to Z'
    m_atTransformMtx[0][2][1] = 0;     // Y to Z'
    m_atTransformMtx[0][2][2] = 0;     // Z to Z'
    m_atTransformMtx[0][2][3] = 0;     // Z0
...
int CLibertyTrack::SetSensorsTransformIdx(  int nSensors, int * piTransformIdx ){
  ...
  if( nSensors != m_nActiveSensors ){
    CP_printf("\nCannot set sensor transforms. Wrong input number of sensors: %d", nSensors);
  ...
  for( iFrame = 0; iFrame < nFrames; iFrame++ ) {
    for (iSensor = 0; iSensor < m_nActiveSensors; iSensor++){
      ...
      if(iSensor == iSensorIdx){
        ...
        m_TransformCoordinates( iTrans, &ptFrame->tfCoord, pfCoordinates++); // coordinates
        memcpy(&ptFrame->tfAngles, pfCoordinates++, sizeof(tfXYZ));          // angles
        ZeroMemory(pfVelocities++, sizeof(tfXYZ));
      }
      ptFrame++;
    }
  }
...
int CLibertyTrack::m_GetNewData( PBYTE pBuf, long lngN_Bytes){
  ...
  if( lngN_Bytes != nFrameSize * m_nActiveSensors ){
    CP_printf("\n\nError! Data size mismatch:\n");
    CP_printf("Expected block size: %d   Actual block size: %d\n",  
                             nFrameSize * m_nActiveSensors, lngN_Bytes);
    CP_printf("Stopping acquisition.\n");
    m_DisconnectLiberty();
    return 2;
  }
  pLibertyFrame = (tLibertyFrame *) pBuf;  // pointer to the first frame
