...
#define LIBERTY_BUFFER_SIZE_SECONDS 10
...
#define PNO_BUF_SIZE  (sizeof(tLibertyFrame) * N_SENSORS_MAX * FRAME_RATE_HZ * LIBERTY_BUFFER_SIZE_SECONDS) 
...
//  tLibertyFrame m_LatestFrames[N_SENSORS_MAX]; // Liberty->SlideBuffer tmp storage 
  ...
  void m_TransformAngles( int iTrans, tfXYZ * ptInXYZ,   tdXYZ * ptOutXYZ );
...
public:
  ...
  int GetLibertyResult( char ** pszResult, int * piResult  );
  int GetSignalDistortion( int aiDistortion[N_SENSORS_MAX] );

  static void OnDataStart( CLibertyTrack * pLT, 
                         WPARAM wParam, 
                         LPARAM lParam );
