...
//==============================================================================
int CLibertyTrack::GetFrameCount( int *pnFrames){

  // The first frame has index 1
  // This way it is compatible with the frame counter hardware
  if( m_iFirstFrameIdx ) { // we have frames in the buffer
    *pnFrames = m_iLibertyFrameIdx - m_iFirstFrameIdx + 1;
  }
  else {  // no frames yet
    *pnFrames = 0;
  }
...
int CLibertyTrack::FinishAndDisconnect(){
  ...
  if( m_iIsRunning ) {
    m_iIsRunning = 0;
   // stop frame marking to stop DAQ
    m_PdiDev.SetSyncEnabled ( FALSE ); 
  }
  else{
...
//===========================================================================
void CLibertyTrack::OnDataStart( CLibertyTrack * pLT, 
                                       WPARAM wParam, 
                                       LPARAM lParam ){
  pLT->m_StartData(); 
...
int CLibertyTrack::m_GetNewData( PBYTE pBuf, long lngN_Bytes){
  ...
  blnMarkedFrame = pLibertyFrame->uiExternalSync;

  // if we have no more marked frames, stop DAQ, disconnect Liberty
  if(!m_iIsRunning  && !blnMarkedFrame ) {  
    m_DisconnectLiberty();
  }

  // If we get spurious marked frames in the beginning, skip them
  if( blnMarkedFrame && !m_nUnmarkedFrames) {
    CP_printf("Skipping spurious marked frames.\n");
    return 0;
  }

  // In the beginning get 10 marked frames then turn the marking on
  if( !blnMarkedFrame ) {
    m_nUnmarkedFrames++;
    // when we had 10 unmarked frames, start marking them
    if(m_nUnmarkedFrames == 10) {
      m_PdiDev.SetSyncEnabled ( TRUE ); // Enable output TTL sync 
      CP_printf("Frame marking enabled.\n");
    }
    return 0;
  }

  m_iLibertyFrameIdx = pLibertyFrame->uiFrameIdx;
  // If it is the first marked frame, save its index - it is an offset to
  // the count of pulses going to RP2 counter 
  if( m_nMarkedFrames == 0 ) {
    m_iFirstFrameIdx = m_iLibertyFrameIdx;
    CP_printf("Got %d unmarked frames.\n", m_nUnmarkedFrames);
    CP_printf("First marked frame index: %d\n", m_iFirstFrameIdx);
  }
  m_nMarkedFrames++;

  // Check that the frames are consecutive
  nMissed = (m_iLibertyFrameIdx - m_iPrevFrame) -1;
  if( nMissed && (m_nMarkedFrames>1)) {
    CP_printf("\nMissed  %d  frames.\n", nMissed);
