...
int  Tserial::connect(char *port_arg, int rate_arg, serial_parity parity_arg)
{
    ...
    if (port_arg!=0)
    {
        ...
        if (serial_handle    != INVALID_HANDLE_VALUE)
        {
            ...
            cto.ReadIntervalTimeout = 0;  // max inter-char interval in ms, 0==unused
            cto.ReadTotalTimeoutMultiplier = PER_CHAR_TIMEOUT_MS;  // max per-char delay (ms) in array reads
