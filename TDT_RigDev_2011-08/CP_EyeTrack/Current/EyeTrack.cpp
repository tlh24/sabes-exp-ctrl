...
int CEyeTrack::ConnectISCAN(  ){
  ...
  if( iRet ) {
    m_iPortIO_Granted = 0;
    CP_printf("Error: Cannot grant I/O access, error: %d\n", iRet );
    return 1;
  }
  ...
  // Reset the counter
  // Note!!!!
  // it looks like we have to make two reset pulses,
  // otherwise reset may fail
...
int CEyeTrack::DisconnectISCAN(){
  ...
  if( m_iPortIO_Granted ) {
    PortOutput( LPT_CONTROL, GATE_OFF);
    Sleep(100);  // wait for the counters to update
    CP_printf("Final frame count: %d\n", m_iPrevFrame);
  }
  //Terminate thread
...
//==============================================================================
int CEyeTrack::GetFrameCount( int * pnFrames){

  *pnFrames = m_iPrevFrame;
  return 0;
}
...
int CEyeTrack::m_WriteData( int * piItems, int nItems ){
  ...
  if( nItems > 2 ) {
    ...
    iDigIn = ( (piItems[2]/10) & 0xF );  // get only 4 bits of the counter
