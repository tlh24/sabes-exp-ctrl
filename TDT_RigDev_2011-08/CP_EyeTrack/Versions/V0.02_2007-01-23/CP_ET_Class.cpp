...
HRESULT CP_ET_Class::GetLatestRawData( long *plIdx, double *pdTimeMs, long *plX, long *plY) {
  ...
  if( g_pEyeTrack->ReadLatestItem( &iIdx, &ScanData ) ){
    * plIdx = iIdx;
    * pdTimeMs = ScanData.dTimeStampMs; 
    * plX = ScanData.iRawX; 
    * plY = ScanData.iRawY;
//    CP_printf("%f\t%d\t%d\n", *pdTimeMs, *plX, *plY );
    return S_OK;
...
HRESULT CP_ET_Class::GetLatestData( long *plIdx, SAFEARRAY ** ppadData ) {
  ...
  *ppadData = NULL;
  ...
  // Timestamp
  rgIndices[1] = 0;
  SafeArrayPutElement( padData, rgIndices, &ScanData.dTimeStampMs );
  // raw X-coord
  rgIndices[1] = 1;
...
HRESULT CP_ET_Class::GetDataBuffer( long lngIdx,
  ...
  *ppadData = padData;
  ...
  for( i = 0; i<nItems; i++ ){
    ...
    // Timestamp
    rgIndices[1] = 0;
    SafeArrayPutElement( padData, rgIndices, &pScanData[i].dTimeStampMs );
    // raw X-coord
    rgIndices[1] = 1;
    SafeArrayPutElement( padData, rgIndices, &dRawX );
    // raw Y-coord
